 ...
 * Product Loop Start
 *
 * This template can be overridden by copying it to yourtheme/woocommerce/loop/item.php.
 *
...
// Check if product is in favorites (if using WC Wishlist or similar plugin)
$is_favorite = false;
if (function_exists('wc_wishlists_get_wishlists')) {
    $wishlists = wc_wishlists_get_wishlists(array('user_id' => get_current_user_id()));
...
<div class="nxt-product-card" data-product-id="<?php echo esc_attr($product_id); ?>">
    ...
    <div class="nxt-product-card__image-container">
        ...
        <!-- Favorite Button -->
        <div class="nxt-product-card__favorite-btn <?php echo $is_favorite ? 'is-favorite' : ''; ?>"
            data-product-id="<?php echo esc_attr($product_id); ?>"
            aria-label="<?php esc_attr_e('Add to favorites', 'hello-elementor-child'); ?>">
            ...
            <?php echo do_shortcode('[nxt_wishlist_button product_id="' . $product_id . '"]'); ?>
        </div>

        <!-- Add to cart button -->
        <button class="nxt-product-card__add-btn"
            data-product-id="<?php echo esc_attr($product_id); ?>"
            aria-label="<?php esc_attr_e('Add to cart', 'hello-elementor-child'); ?>">
            <svg class="nxt-product-card__add-icon" width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
